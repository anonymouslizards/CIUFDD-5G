...
Due to repository size constraints, the complete generated datasets and PCAP files are not included; representative samples are provided in labeled datasets.zip and pcaps.zip. The simulation and processing scripts, configurations, deterministic device placement, and fixed simulation seeds allow the full set of 90 runs to be regenerated.
